...
- `receiverTimestamp`: the UNIX time at which the waku message is received by the receiving node.
- `senderTimestamp`: the UNIX time at which the waku message is generated by its sender.
...
The queried node MUST sort the `WakuMessage`s based on their `Index`, where the `senderTimestamp` constitutes the most significant part and the `digest` comes next, and then perform pagination on the sorted result. As such, the retrieved page contains an ordered list of `WakuMessage`s from the oldest message to the most recent one.

Alternatively, the `receiverTimestamp` (instead of `senderTimestamp` ) can be used to sort `WakuMessage`s during the paging process. 
However, we RECOMMEND the use of the `senderTimestamp` for sorting as it is invariant and consistent across all the nodes.
