...
- `timestamp`: the UNIX time at which the waku message is generated by its sender.
...
The queried node MUST sort the `WakuMessage`s based on their `Index`, where the `timestamp` constitutes the most significant part and the `digest` comes next, and then perform pagination on the sorted result. As such, the retrieved page contains an ordered list of `WakuMessage`s from the oldest message to the most recent one.
